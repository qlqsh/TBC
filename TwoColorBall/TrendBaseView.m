...
- (void)setWinningReds:(NSArray *)reds andBlues:(NSArray *)blues {
	for (NSString *red in reds) {
		NSInteger value = [red integerValue];
		BallView *ballView = (BallView *) _ballViews[(NSUInteger) (value - 1)];
		ballView.state = 2;
		ballView.textLabel.text = red;
	}
	for (NSString *blue in blues) {
		NSInteger value = [blue integerValue];
		BallView *ballView = (BallView *) _ballViews[(NSUInteger) (33 + value - 1)];
		ballView.state = 3;
