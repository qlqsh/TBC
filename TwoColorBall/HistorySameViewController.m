...
- (void)viewWillAppear:(BOOL)animated {
	...
	// 预设一下行高
	_heightOfCell = [WinningBallView heightOfCellWithWidth:(CGFloat) (self.view.bounds.size.width * 0.75)];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil) {
		...
		UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(kDefaultSpace,
				kDefaultSpace, (CGFloat) (width * 0.2), _heightOfCell)];
		textLabel.tag = 101;
		...
		// 解决重用导致的重影问题
		WinningBallView *winningBaseView =
				[[WinningBallView alloc] initWithFrame:CGRectMake((CGFloat) (width * 0.2 + kDefaultSpace),
						kDefaultSpace, (CGFloat) (width * 0.75), _heightOfCell)];
