...
- (PNPieChart *)drawPieChartWithArray:(NSArray *)dataArray {
	...
	CGFloat width = (CGFloat) (self.view.bounds.size.width * 0.8);
	CGFloat startXY = (CGFloat) (self.view.bounds.size.width * 0.05);
...
// 这个方式是有问题的，就是数据特别少的情况下，会崩溃。不过那种情况是以后才用，暂时不会有问题。
- (PNPieChart *)drawHeadOrTailPieChartWithArray:(NSArray *)dataArray {
	NSMutableArray *items = [NSMutableArray arrayWithCapacity:dataArray.count];
	NSArray *colorArray = @[kRedColor, kBlueColor, kGreenColor, kOrangeColor, kYellowColor, kCyanColor];
	for (NSUInteger i = 0; i < 5; i++) {
		NumberCombinations *numberCombinations = dataArray[i];
		PNPieChartDataItem *dataItem = [PNPieChartDataItem dataItemWithValue:numberCombinations.showNumber
																	   color:colorArray[i]
																 description:numberCombinations.toString];
		[items addObject:dataItem];
	}
	int other = 0;
	for (NSUInteger i = 5; i < dataArray.count; i++) {
		NumberCombinations *numberCombinations = dataArray[i];
...
- (PNPieChart *)drawPieChartWithValueArray:(NSArray *)valueArray
	...
	for (NSUInteger i = 0; i < count; i++) {
		PNPieChartDataItem *dataItem = [PNPieChartDataItem dataItemWithValue:[valueArray[i] integerValue]
																	   color:colorArray[i]
...
- (UICollectionViewFlowLayout *)collectionViewFlowLayout {
	if (!_collectionViewFlowLayout) {
		...
		// 设置cell
		CGFloat width = (CGFloat) (self.view.bounds.size.width * 0.9);
